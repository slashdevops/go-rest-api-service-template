...
func TestPasswordResetTokenValidator_GetClientID(t *testing.T) {
	validator := PasswordResetTokenValidator{ClientID: "test-client-id"}

	assert.Equal(t, "test-client-id", validator.GetClientID())
}

func TestPasswordResetTokenValidator_Validate(t *testing.T) {
	...
	t.Run("verifies_through_the_signer_and_returns_the_claims", func(t *testing.T) {
		...
		validator := PasswordResetTokenValidator{Verifier: verifier, ClientID: "test-client-id"}
	...
	t.Run("refuses_what_the_signer_refuses", func(t *testing.T) {
		validator := PasswordResetTokenValidator{
	...
	t.Run("requires_a_jti_because_a_reset_token_can_be_revoked", func(t *testing.T) {
		claims := validatorClaims()
		delete(claims, "jti")

		validator := PasswordResetTokenValidator{Verifier: &fakeVerifier{claims: claims}, ClientID: "test-client-id"}

		_, err := validator.Validate(ctx, "the-token")
		assert.Error(t, err, "a reset token that cannot be named cannot be revoked")
	})
}
