...
type IDPResponse struct {
	...
	CallbackURL         string           `json:"callback_url,omitzero" example:"https://example.com/callback" format:"uri"`          // OAuth callback URL
	LoginRedirectURL    string           `json:"login_redirect_url,omitzero" example:"https://example.com/login" format:"uri"`       // Login redirect URL
	RegisterRedirectURL string           `json:"register_redirect_url,omitzero" example:"https://example.com/register" format:"uri"` // Registration redirect URL
...
type CreateIDPRequest struct {
	...
	LoginRedirectURL    string    `json:"login_redirect_url" example:"https://example.com/login" format:"uri" validate:"required"`                                                     // Login redirect URL
	RegisterRedirectURL string    `json:"register_redirect_url" example:"https://example.com/register" format:"uri" validate:"required"`                                               // Registration redirect URL
...
func (ref *CreateIDPRequest) Validate() error {
	...
	for field, val := range map[string]string{
		...
		domain.FieldLoginRedirectURL:    ref.LoginRedirectURL,
		domain.FieldRegisterRedirectURL: ref.RegisterRedirectURL,
...
type UpdateIDPRequest struct {
	...
	LoginRedirectURL    *string    `json:"login_redirect_url,omitempty" example:"https://example.com/login" format:"uri" validate:"optional"`                                   // Updated login redirect URL
	RegisterRedirectURL *string    `json:"register_redirect_url,omitempty" example:"https://example.com/register" format:"uri" validate:"optional"`                             // Updated registration redirect URL
...
func (req *UpdateIDPRequest) Validate() error {
	...
	stringChecks := []struct {
		...
		{domain.FieldLoginRedirectURL, req.LoginRedirectURL, 5, 255, true},
		{domain.FieldRegisterRedirectURL, req.RegisterRedirectURL, 5, 255, true},
	}
