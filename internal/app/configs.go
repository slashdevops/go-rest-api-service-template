...
func setupFlags(configs *Configs) {
	...
	flag.IntVar(&configs.HTTPServer.MaxBodyBytes.Value, configs.HTTPServer.MaxBodyBytes.FlagName, config.DefaultHTTPServerMaxBodyBytes, configs.HTTPServer.MaxBodyBytes.FlagDescription)
	flag.IntVar(&configs.HTTPServer.MaxBodyBytesIngest.Value, configs.HTTPServer.MaxBodyBytesIngest.FlagName, config.DefaultHTTPServerMaxBodyBytesIngest, configs.HTTPServer.MaxBodyBytesIngest.FlagDescription)
	flag.BoolVar(&configs.HTTPServer.HSTSEnabled.Value, configs.HTTPServer.HSTSEnabled.FlagName, config.DefaultHTTPServerHSTSEnabled, configs.HTTPServer.HSTSEnabled.FlagDescription)
	flag.DurationVar(&configs.HTTPServer.HSTSMaxAge.Value, configs.HTTPServer.HSTSMaxAge.FlagName, config.DefaultHTTPServerHSTSMaxAge, configs.HTTPServer.HSTSMaxAge.FlagDescription)
	flag.StringVar(&configs.HTTPServer.PublicURL.Value, configs.HTTPServer.PublicURL.FlagName, config.DefaultHTTPServerPublicURL, configs.HTTPServer.PublicURL.FlagDescription)
	...
	flag.BoolVar(&configs.HTTPServer.SwaggerEnabled.Value, configs.HTTPServer.SwaggerEnabled.FlagName, config.DefaultHTTPServerSwaggerEnabled, configs.HTTPServer.SwaggerEnabled.FlagDescription)
	...
	flag.BoolVar(&configs.HTTPClient.AllowPrivateAddresses.Value, configs.HTTPClient.AllowPrivateAddresses.FlagName, config.DefaultHTTPClientAllowPrivateAddresses, configs.HTTPClient.AllowPrivateAddresses.FlagDescription)
	...
	flag.IntVar(&configs.Authn.PasswordBcryptCost.Value, configs.Authn.PasswordBcryptCost.FlagName, config.DefaultAuthnPasswordBcryptCost, configs.Authn.PasswordBcryptCost.FlagDescription)
	flag.BoolVar(&configs.Authn.SeedAdminPasswordAllowed.Value, configs.Authn.SeedAdminPasswordAllowed.FlagName, config.DefaultAuthnSeedAdminPasswordAllowed, configs.Authn.SeedAdminPasswordAllowed.FlagDescription)
