...
- **The operation reaches every record from the context, not from the call site.** `o11y.SetupTrace` puts it there and `operationAttrsHandler` adds `app_layer`/`app_domain`/`app_action` to every line written inside it — it wraps the COMPOSED logger, because wrapping only the standard handler sends them to stdout and not to the log store, and the bridge supplying `trace_id` on the exported side hides that. **A message says what happened, never which operation**; the operation is attributes, and naming it in the message is the same fact twice, once filterable and once not.
